...
- **Global scope**: `E:/project/symforge/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/project/symforge/.claude/notes/`)
...
2. Glob `E:/project/symforge/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/project/symforge/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/project/symforge/.claude/notes/` — works in any directory
