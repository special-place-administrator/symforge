...
pub fn plan_edit(index: &LiveIndex, target: &str) -> String {
    ...
    if !symbol_hits.is_empty() {
        lines.push(format!("Found {} symbol(s) matching '{}':", symbol_hits.len(), target));
        for (path, name, kind, (start, end)) in &symbol_hits {
            lines.push(format!("  {:?} {} in {} (lines {}-{})", kind, name, path, start, end));
        }

        // Count references
        let ref_count = index.find_references_for_name(target, None, false).len();
        lines.push(format!("\nReferences: {} call sites across the project", ref_count));

        if ref_count > 10 {
            lines.push("⚠ HIGH IMPACT: >10 callers. Use batch_rename(dry_run=true) to preview.".to_string());
        }
        ...
        if symbol_hits.len() == 1 {
            ...
            lines.push(format!("     - Delete: delete_symbol(path=\"{path}\", name=\"{name}\", dry_run=true)"));
            lines.push(format!("  3. analyze_file_impact(path=\"{path}\") — verify changes"));
        } else {
    ...
    if let Some(path) = file_hit {
        if symbol_hits.is_empty() {
            ...
            lines.push(format!("  2. get_symbol(path=\"{path}\", name=\"<target>\") — read specific symbols"));
            lines.push(format!("  3. Use edit_within_symbol or replace_symbol_body for changes"));
            lines.push(format!("  4. analyze_file_impact(path=\"{path}\") — verify"));
        }
    }
